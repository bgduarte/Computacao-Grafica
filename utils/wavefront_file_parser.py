...
class WavefrontFileParser:
    ...
    @staticmethod
    def __load_wavefront_file(filepath: str) -> Tuple[List[Coordinate3D], List[MaterialObject], List[List[str]]]:
        ...
        vertices = [Coordinate3D(list(map(float, v.split()[1:4]))) for v in V]
    ...
    @staticmethod
    def __wavefront_obj_to_displayable(obj: WavefrontObject) -> Displayable:
        color = WavefrontFileParser.__rgb_float_to_hex(obj.material.diffuse_color)
        coords = [Coordinate2D(v.x, v.y) for v in obj.vertices]
        if len(obj.vertices) == 1:
            return Dot(obj.name, color, coords)
        elif len(obj.vertices) == 2:
            return Line(obj.name, color, coords)
